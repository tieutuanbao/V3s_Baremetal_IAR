...
void PLL_Init(PLL_CTRL_Enum pll, PLL_PARAM_TypeDef pllParam)
{
	...
	switch(pll){
		...
		case PLL_VIDEO:{
			...
			temp = CCU_PLL_VIDEO_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | (pllParam.M - 1);
			CCU_PLL_VIDEO_CTRL = temp;
		...
		case PLL_VE:{
			...
			temp = CCU_PLL_VE_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | (pllParam.M - 1);
			CCU_PLL_VE_CTRL = temp;
		...
		case PLL_PERIPH0:{
			...
			temp = CCU_PLL_PERIPH0_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | ((pllParam.K - 1) << 4) | (pllParam.M - 1);
			CCU_PLL_PERIPH0_CTRL = temp;
		...
		case PLL_ISP:{
			...
			temp = CCU_PLL_ISP_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | (pllParam.M - 1);
			CCU_PLL_ISP_CTRL = temp;
		...
		case PLL_PERIPH1:{
			...
			temp = CCU_PLL_PERIPH1_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | ((pllParam.K - 1) << 4) | (pllParam.M - 1);
			CCU_PLL_PERIPH1_CTRL = temp;
		...
		case PLL_DDR1:{
			...
			temp = CCU_PLL_DDR1_CTRL;
			temp |= (1U << 31);					// Enable PLL
			temp |= ((pllParam.N - 1) << 8) | (pllParam.M - 1);
			CCU_PLL_DDR1_CTRL = temp;
			while((CCU_PLL_DDR1_CTRL & CCU_PLL_LOCKED_MASK) == 0);
			break;
		}
	}
	temp = CCU_CPU_AXI_CFG;
	temp &= ~(0x03 << 16);
	temp |=  (0x02 << 16);
	CCU_CPU_AXI_CFG = temp;	
}
